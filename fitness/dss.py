# MODIFIED FROM: https://github.com/photosynthesis-team/piq/blob/1fa3923dc6f7b13c05e5f82f5408b7419b4e04bf/piq/dss.py
...
def _subband_similarity(x: torch.Tensor, y: torch.Tensor, first_term: bool,
    ...
    kernel = gaussian_filter(kernel_size=float(kernel_size), sigma=sigma)
    ...
    sigma_xx = torch.max(sigma_xx, torch.tensor(1e-10).to(sigma_xx))
    sigma_yy = torch.max(sigma_yy, torch.tensor(1e-10).to(sigma_yy))
